...
bool is_primitive_root(long long g, long long p) {
    if (!is_prime(p) || gcd(g, p) != 1) return false;
    long long phi = p - 1;
...
void elgamal(long long p, long long g, long long a, long long m, long long k) {
    ...
    if (!is_prime(p)) { printf("Invalid: p must be prime\n"); return; }
    if (!is_primitive_root(g, p)) { printf("Invalid: g must be a primitive root of p\n"); return; }
    if (a < 1 || a > p - 2) { printf("Invalid: private key a must be in range [1, p-2]\n"); return; }
    if (m < 0 || m >= p) { printf("Invalid: message m must be in range [0, p)\n"); return; }
    if (k < 1 || k > p - 2) { printf("Invalid: random k must be in range [1, p-2]\n"); return; }
